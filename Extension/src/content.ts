...
// Интерфейс для сообщений между контент-скриптом и попапом
interface Message {
    type: 'START_OPTIMIZATION' | 'STOP_OPTIMIZATION' | 'OPTIMIZATION_PROGRESS' | 'OPTIMIZATION_COMPLETE';
    data?: any;
}

class ContentScript {
    private optimizer: TradingViewOptimizer | null = null;
    ...
    private async handleMessage(message: Message, sendResponse: (response?: any) => void): Promise<void> {
        switch (message.type) {
            case 'START_OPTIMIZATION':
                await this.startOptimization(message.data, sendResponse);
                break;
    ...
    private async startOptimization(
        data: { params: OptimizationParams; ranges: { [key: string]: { min: number; max: number; step: number } } },
        sendResponse: (response?: any) => void
    ): Promise<void> {
        try {
            if (this.isOptimizing) {
                sendResponse({ error: 'Оптимизация уже запущена' });
                return;
            }

            this.isOptimizing = true;

            // Получаем текущий символ из URL TradingView
            const symbol = this.getCurrentSymbol();
            if (!symbol) {
                throw new Error('Не удалось определить текущий символ');
            }

            // Инициализируем оптимизатор
            this.optimizer = new TradingViewOptimizer();
            await this.optimizer.initialize(symbol);

            // Запускаем оптимизацию
            const result = await this.optimizer.optimize(data.params, data.ranges);

            // Отправляем результат в попап
            chrome.runtime.sendMessage({
            ...
            console.error('Ошибка при запуске оптимизации:', error);
    ...
    private getCurrentSymbol(): string | null {
        // Получаем символ из URL TradingView
        const match = window.location.href.match(/chart\/([^/]+)/);
        return match ? match[1] : null;
    }
}

// Создаем экземпляр контент-скрипта
new ContentScript();
